...
- [DB-16: Autonomous Cycle Model](./decision-boundaries/16-autonomous-cycle-model.md) — **DECIDED**: Full milestone execution without human interruption, output-verification over plan-approval, structured re-entry via ask_user
- [DB-17: Brutalist Verification Architecture](./decision-boundaries/17-brutalist-verification-architecture.md) — **DECIDED**: Brutalist as external multi-model verification, separate from coordinator self-assessment
- [DB-18: Memory Architecture](./decision-boundaries/18-memory-architecture.md) — **DECIDED**: `memory.md` (operational memory distinct from understanding.md), orchestrator-primary consolidation at milestone boundaries, milestone-distance decay with temporal invalidation, rule-based content-type-first scored retrieval
